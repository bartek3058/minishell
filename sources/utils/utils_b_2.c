...
int	execute_pipe_sequence(t_minishell *shell, t_command *start_cmd, t_token **token, char **args)
{
	...
	{
		ft_execute_pipe(shell, start_cmd, start_cmd->next, token, args);
		return (shell->exit_status);
	}
	else
	{
...
int	execute_command_chain(t_minishell *shell, t_command *cmd_list, t_token **token, char **args)
{
	...
	while (current)
	{
		if (current->pipe_out)
		{
			last_exit_status = execute_pipe_sequence(shell, current, token, args);
			current = skip_pipe_sequence(current);
		}
		else
			last_exit_status = execute_single_command(shell, current, token, args);
